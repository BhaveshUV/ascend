...
const AllListings = () => {
    ...
    return !listings ? <div>Fetching listings...</div> : isForm ? <EditListing listing={{}} setIsForm={setIsForm} /> : (
        <>
            <div className="flex gap-4 items-end my-8 px-4">
                <span className="text-3xl font-bold">All Listings</span>
                <button onClick={() => setIsForm(true)} className="rounded px-2 border-2 h-7 bg-zinc-100 hover:bg-zinc-200 cursor-pointer">Add new Listing</button>
            </div>
